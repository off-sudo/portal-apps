...
function getStatusStyle(status: string | null): { bg: string; color: string; label: string } {
  const s = (status || '').toLowerCase();
  if (s === 'in_progress') return { bg: '#22c55e18', color: '#22c55e',  label: 'LIVE' };
  if (s === 'confirmed')   return { bg: '#2563eb15', color: '#2563eb',  label: 'CONFIRMED' };
  if (s === 'pending')     return { bg: '#f59e0b18', color: '#f59e0b',  label: 'PENDING' };
  if (s === 'completed')   return { bg: '#e0e7ef',   color: '#6b8ab8',  label: 'DONE' };
  if (s === 'cancelled')   return { bg: '#fee2e2',   color: '#dc2626',  label: 'CANCELLED' };
  return { bg: '#e0e7ef', color: '#6b8ab8', label: (status || 'unknown').toUpperCase() };
}
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: bg }]}>
      ...
      <ScrollView
        contentContainerStyle={[
          ...
          {
            // Extra top buffer: keeps avatar away from status bar on Android
            paddingTop: Platform.OS === 'android'
              ? (StatusBar.currentHeight ?? 24) + 12
              : 20,
            // Extra bottom buffer: clears nav bar / home indicator
        ...
        <View style={styles.profileHero}>
          ...
          {profileLoading ? (
            <ActivityIndicator size="small" color={accentBlue} style={{ marginTop: 8 }} />
          ) : (
            <>
              <Text style={[styles.heroName, { color: text }]}>
                {profile?.full_name || 'Your Name'}
        ...
        {/* ── Stats Row ── */}
        <View style={styles.statsRow}>
          <StatCard label="Total trips" value={bookings.length}        bg={card} border={border} text={text} muted={muted} accent={accentBlue} />
          <StatCard label="Active"      value={activeBookings.length}  bg={card} border={border} text={text} muted={muted} accent="#22c55e" />
          <StatCard
            label="Spent"
            value={'₹' + bookings.reduce((s, b) => s + (b.fare ?? 0), 0)}
            bg={card} border={border} text={text} muted={muted}
        ...
        <View style={[styles.bookingTabRow, { backgroundColor: card, borderColor: border }]}>
          {(['active', 'past'] as TabKey[]).map((tab) => (
            <Pressable
              ...
              style={[
                styles.bookingTab,
                bookingTab === tab && [styles.bookingTabActive, { borderBottomColor: accentBlue }],
              ]}
              onPress={() => setBookingTab(tab)}>
              <Text style={[styles.bookingTabText, { color: bookingTab === tab ? accentBlue : muted }]}>
                {tab === 'active' ? 'Active' : 'Past'}
              </Text>
              <View style={[
                styles.tabCountBadge,
                { backgroundColor: bookingTab === tab ? accentBlue : muted },
              ]}>
                <Text style={styles.tabCountText}>
                  {tab === 'active' ? activeBookings.length : pastBookings.length}
                </Text>
...
function BookingCard({
  ...
  return (
    <View style={[styles.bookingCard, { backgroundColor: card, borderColor: border }]}>
      <View style={styles.cardTop}>
        <View style={[styles.vehicleBox, { backgroundColor: isDark ? '#1e2d4a' : '#eff6ff' }]}>
          <Text style={{ fontSize: 20 }}>{icon}</Text>
        </View>
        <View style={styles.cardMid}>
          {/* flex:1 on cardMid + flexWrap on routeText ensures long addresses wrap */}
          <Text style={[styles.routeText, { color: text }]}>
            {item.pickup_location || 'Pickup'} → {item.dropoff_location || 'Drop'}
          </Text>
          <Text style={[styles.cardMeta, { color: muted }]}>
            {formatDate(item.pickup_date)} · {item.pickup_time?.slice(0, 5) || 'N/A'} · {item.vehicle_type || 'Vehicle'}
          </Text>
        </View>
        <View style={[styles.statusBadge, { backgroundColor: st.bg }]}>
          <Text style={[styles.statusText, { color: st.color }]}>{st.label}</Text>
      ...
      {item.fare != null && (
        <View style={[styles.cardFareRow, { borderTopColor: isDark ? '#1e2d4a' : '#eff6ff' }]}>
          <Text style={[styles.cardFareLabel, { color: muted }]}>Fare</Text>
          <Text style={[styles.cardFareValue, { color: text }]}>₹{item.fare}</Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  // top/bottom padding applied dynamically above
  ...
  bookingCard: { borderWidth: 1, borderRadius: 16, overflow: 'hidden' },
  cardTop: { flexDirection: 'row', alignItems: 'flex-start', gap: 10, padding: 12 },
  vehicleBox: { width: 44, height: 44, borderRadius: 12, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  // flex:1 constrains the text block so long routes wrap instead of overflow
  cardMid: { flex: 1 },
  ...
  logoutButton: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: '#1e3a5f',   // deep navy instead of orange-red
    marginTop: 6,
  },
  logoutText: { color: '#fff', fontWeight: '800', fontSize: 15 },
});